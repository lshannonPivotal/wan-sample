#!/bin/sh
echo "****Starting The Local Cluster 2 on 52.0.225.28****"

##Set the variables
export HOSTNAME=$(hostname)
export LOCATOR_NAME="$(hostname)_locator"
export SERVER_NAME="$(hostname)_server"
export WORKING_DIRECTORY=/home/ubuntu/wan-sample/gemfire-package2
...
echo "Starting Up Cluster 2 Locator: $LOCATOR_NAME running on port $LOCATOR_PORT"
...
--J='-Dgemfire.remote-locators=172.31.38.93[10334]' \
--J='-Dgemfire.distributed-system-id=2'

##Create the member directory for the server
...
echo "****Local Cluster 2 Started on 52.0.225.28****"
